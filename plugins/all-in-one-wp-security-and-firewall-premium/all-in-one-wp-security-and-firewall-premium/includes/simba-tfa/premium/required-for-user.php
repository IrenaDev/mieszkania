...
class Simba_Two_Factor_Authentication_Required_For_User {
	...
	public function action_init() {
		...
		if (is_admin() && apply_filters('simba_tfa_apply_redirect_in_admin', true)) return;
	...
	public function simba_tfa_after_user_roles($default) {
		...
		wp_localize_script(
			...
			apply_filters('simba_tfa_require_for_user_localize', array(
				...
				'required_for_role' => __('(TFA is configured to not be available for this role)', 'all-in-one-wp-security-and-firewall-premium')
			))
		);
		...
		foreach($wp_roles->role_names as $id => $name) {	
			...
			$ret .= '<input type="checkbox" id="tfa_required_'.$id_esc_attr.'" name="tfa_required_'.$id_esc_attr.'"  class="tfa_pre_requisite_role_'.$id_esc_attr.'" value="1" '.($setting ? 'checked="checked"' : '').'> <label id="tfa_required_'.$id_esc_attr.'_label" for="tfa_required_'.$id_esc_attr.'">'.htmlspecialchars($name).' <span class="label_append"></span></label><br>'."\n";
		}
		...
		$ret .= sprintf(__('Enforce this requirement only for accounts at least %s days old', 'all-in-one-wp-security-and-firewall-premium'), '<input type="number" style="width:80px;" step="1" min="0" name="tfa_requireafter" id="tfa_requireafter" value="'.$requireafter.'">');
		
		$require_enforce_after = $this->simba_tfa->get_option('tfa_require_enforce_after');
		
		$ret .= '<br>'.sprintf(__('Do not begin enforcing this setting until: %s', 'all-in-one-wp-security-and-firewall-premium'), '<input type="text" style="width: 120px;" name="tfa_require_enforce_after" id="tfa_require_enforce_after">').'<br>'.__('(If you are setting up for the first time and have pre-existing users, then you may also want to inform them).', 'all-in-one-wp-security-and-firewall-premium');;		
		...
		$ret .= '<br>'.__('If TFA is not active on a required account, then allow them to login but redirect them from any other page to (e.g. your preferred page where they can set it up - make sure it is the WordPress canonical URL for the page so that a redirect loop is not set up):', 'all-in-one-wp-security-and-firewall-premium');
