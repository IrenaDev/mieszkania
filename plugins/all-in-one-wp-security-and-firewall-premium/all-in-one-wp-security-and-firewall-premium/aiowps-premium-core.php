...
class AIOWPS_PREMIUM {
    ...
    public function __construct() {
		...
		add_action('init', array($this, 'aiowps_cb_plugin_init'), 1);
	...
	public function includes() {
		...
		if (is_admin()) { // Load admin side only files
			...
			include_once('admin/aiowps-cb-admin-init.php');
	...
	public function loader_operations() {
		...
		if (is_admin()) {
			...
			$this->admin_init = new AIOWPS_CB_Admin_Init();
                    ...
                    $this->is_aiowps_admin_page())
                {
                    echo '<div class="notice notice-error">
                            <p>';
                                ...
                                printf(
                                __('The %s file is not readable. Ensure that the file exists. If it exists, please ensure that the file path is writable and readable.', 'all-in-one-wp-security-and-firewall-premium'),
	...
	private function create_folders() {
		...
		if (!is_dir($aiowps_premium_geodb_dir) && !wp_mkdir_p($aiowps_premium_geodb_dir)) {
			...
			return new WP_Error('create_folders', sprintf(__('The request to the filesystem failed: unable to create directory %s. Please check your file permissions.'), str_ireplace(ABSPATH, '', $aiowps_premium_geodb_dir)));
		}
		return true;
	}
	...
	public function aiowps_plugin_not_active() {
		...
		$msg = '<p>'.sprintf(__('Attention! You do not have the %s active. The %s plugin needs to be installed and active in order for the %s Premium addon to work!', 'all-in-one-wp-security-and-firewall-premium'), $aiowps_link, $aiowps_plugin_name, $aiowps_plugin_name).'</p>';
		echo '<div class="error fade">'.$msg.'</div>';
	}
	
	public function aiowps_cb_plugin_init() {//Lets run... Main plugin operation code goes here
		$this->country_tasks_obj = new AIOWPS_Country_Tasks();//For country related tasks
		$this->smart_404_tasks_obj = new AIOWPS_Smart404_Tasks();
